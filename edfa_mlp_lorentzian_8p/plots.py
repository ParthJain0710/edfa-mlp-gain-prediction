...
def generate_plots(history, val_y, y_pred, config):
    ...
    axes[1].set_title("MAE"); axes[1].set_xlabel("Epoch"); axes[1].legend(); axes[1].grid(True)
    ...
    for i, ax in enumerate(axes.flatten()[:n_show]):
        ...
        ax.set_title(f"Output {i+1}"); ax.legend(); ax.grid(True)
    ...
    for i, ax in enumerate(axes.flatten()[:n_show]):
        ...
        ax.set_title(f"Output {i+1}"); ax.grid(True)
    ...
    for i, ax in enumerate(axes.flatten()[:n_show]):
        ...
        ax.set_title(f"Output {i+1} Error"); ax.grid(True)
